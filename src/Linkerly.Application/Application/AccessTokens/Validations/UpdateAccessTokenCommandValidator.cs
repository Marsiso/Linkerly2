...
public class UpdateAccessTokenCommandValidator : AbstractValidator<UpdateAccessTokenCommand>
{
	public UpdateAccessTokenCommandValidator()
	{
		RuleFor(token => token.AccessTokenID)
			.GreaterThan(0)
		...
		RuleFor(token => token.Audience)
			...
			.MaximumLength(512)
			.WithMessage("Obecenstvo tokenu může obsahovat nejvýše 512 znaků.");
		...
		When(token => !string.IsNullOrWhiteSpace(token.Picture), () =>
		{
			RuleFor(token => token.Picture)
				...
				.WithMessage("Pole je požadováno.")
				.MaximumLength(2048)
				.WithMessage("Profilový obrázek uživatele může obsahovat nejvýše 2048 znaků.")
				.URL()
